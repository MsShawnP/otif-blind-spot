...
const COLUMNS: Column[] = [
  { key: 'po_number',           label: 'PO #',          nowrap: true,  sortable: true,  render: (r) => r.po_number },
  { key: 'ship_date',           label: 'Ship date',     nowrap: true,  sortable: true,  render: (r) => r.ship_date ?? '—' },
  { key: 'mabd',                label: 'MABD',          nowrap: true,  sortable: true,  render: (r) => r.mabd ?? '—' },
  { key: 'delivery_date',       label: 'Delivery date', nowrap: true,  sortable: true,  render: (r) => r.delivery_date ?? '—' },
            ...
            {paginated.length === 0 ? (
              ...
              paginated.map((row) => (
                <tr key={row.shipment_id} className={rowClass(row)}>
                  {COLUMNS.map((col) => (
                    <td key={col.key} className={`audit-td${col.nowrap ? ' audit-td--nowrap' : ''}`}>
                      {col.render(row)}
                    </td>
                  ))}
                </tr>
